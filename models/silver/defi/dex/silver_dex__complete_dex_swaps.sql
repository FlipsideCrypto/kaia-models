...
WITH dragonswap_v2 AS (
  ...
  SELECT
    ...
    _inserted_timestamp
  FROM
    {{ ref('silver_dex__dragonswap_v2_swaps') }}
...
kaiaswap AS (
  SELECT
    ...
    _inserted_timestamp
  FROM
    {{ ref('silver_dex__kaiaswap_swaps') }}
...
klayswap_v2 AS (
  SELECT
    ...
    _inserted_timestamp
  FROM
    {{ ref('silver_dex__klayswap_v2_swaps') }}
...
dragonswap_v3 AS (
  SELECT
    ...
    _inserted_timestamp
  FROM
    {{ ref('silver_dex__dragonswap_v3_swaps') }}
...
klayswap_v3 AS (
  SELECT
    ...
    _inserted_timestamp
  FROM
    {{ ref('silver_dex__klayswap_v3_swaps') }}
...
capybara AS (
  SELECT
    ...
    _inserted_timestamp
  FROM
    {{ ref('silver_dex__capybara_swaps') }}

{% if is_incremental() and 'capybara' not in var('HEAL_MODELS') %}
WHERE
  _inserted_timestamp >= (
    SELECT
      MAX(_inserted_timestamp) - INTERVAL '{{ var("LOOKBACK", "4 hours") }}'
